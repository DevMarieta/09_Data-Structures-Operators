...
/////
//Logical Assignment Operators
//Управление на конфигурационни обекти
const config = {
  logLevel: null,
...
// Задаване на стойности по подразбиране само ако текущите стойности са null или undefined
config.logLevel ??= 'info';
config.outputFormat ??= 'json';

// Включване на подробен режим, ако не е зададен изрично като false
config.verbose ||= true;

console.log(config);
// Очакван резултат:
// { logLevel: 'info', outputFormat: 'json', verbose: true }
...
function updateUserProfile(newData) {
  ...
  userProfile.age ??= newData.age;
}

updateUserProfile({ name: 'Alice Johnson', email: null });
console.log(userProfile);
// Очакван резултат:
// { name: 'Alice Johnson', email: 'alice@example.com', age: 25 }

// Условно добавяне на свойства към обект
...
function toggleFeature(settings, feature, isEnabled) {
    if (isEnabled) {
        settings[feature] ||= 'enabled';  // Активирай само ако вече не е активирано
    } else {
        settings[feature] &&= 'disabled';  // Деактивирай само ако вече е активирано
    }
}

toggleFeature(systemSettings, 'experimentalFeature', true);
console.log(systemSettings);
// Очакван резултат:
// { darkMode: true, version: '1.4.2', experimentalFeature: 'enabled' }

toggleFeature(systemSettings, 'darkMode', false);
console.log(systemSettings);
// Очакван резултат:
